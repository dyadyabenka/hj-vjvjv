...
IMAGE_QUERY_PROMPT_TEMPLATE = """Вот текст поста для Telegram-канала:

---
{post}
---

Придумай поисковый запрос для стокового фотобанка (Pexels), чтобы подобрать
фотографию, подходящую именно этому посту по смыслу.

Требования:
- запрос на английском языке, 2-4 слова
- описывай то, что должно быть ВИДНО НА ФОТО (конкретный объект или сцена),
  а не абстрактную идею: не "aging research", а "space station orbit"
  или "laboratory mice cage"
- фото должно быть реалистично найти в стоковом банке: обычные предметы,
  люди, места, лаборатории, еда, природа, техника
- никаких кавычек, пояснений и знаков препинания

Формат ответа: только сам запрос, одной строкой."""
...
class Synthesizer:
    ...
    def image_query(self, post_text: str) -> str | None:
        """Придумывает англоязычный запрос к фотобанку по смыслу самого поста.

        Заменяет статический словарь ключевых слов в images.py: тот работал
        только для заранее перечисленных тем, а на всём остальном
        (например, пост про мышей на МКС) молча скатывался к запасному
        запросу канала — отсюда и брались картинки не по теме.

        None означает "не получилось" — вызывающий код тогда откатывается
        на старый способ подбора.
        """
        prompt = IMAGE_QUERY_PROMPT_TEMPLATE.format(post=post_text)
        result = self._complete(prompt)
        if result is None:
            return None

        # Модель иногда добавляет кавычки/точку вопреки промпту — подчищаем,
        # а на всякий случай отсекаем слишком длинный ответ (Pexels плохо
        # ищет по целым предложениям).
        query = result.strip().strip('".\'').replace("\n", " ").strip()
        if not query or len(query.split()) > 6:
            log.warning("Модель вернула неподходящий запрос для картинки: %r", result[:80])
            return None

        log.info("Запрос для картинки от модели: %s", query)
        return query
